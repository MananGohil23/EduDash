...
const Sidebar = ({ open, onClose }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-slate-200 bg-white transition-transform duration-300 ease-in-out dark:border-slate-800 dark:bg-slate-900 lg:translate-x-0 ${
      ...
      >
        <div className="flex h-20 items-center justify-between px-6">
          ...
          <button
            className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-200 lg:hidden"
            onClick={onClose}
        ...
        <nav className="flex-1 overflow-y-auto px-4 pb-4">
          <p className="px-3 pb-2 pt-2 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">
            Menu
          </p>
                    ...
                    <>
                      <Icon
                        className={`h-5 w-5 ${
                          isActive
                            ? "text-white"
                            : "text-slate-400 group-hover:text-brand-600 dark:text-slate-500 dark:group-hover:text-brand-300"
                        }`}
                      />
                      {label}
                    </>
        ...
        <div className="border-t border-slate-100 p-4 dark:border-slate-800">
          <div className="mb-3 flex items-center gap-3 rounded-2xl bg-slate-50 p-3 dark:bg-slate-800">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brand-100 font-bold text-brand-700 dark:bg-brand-900/50 dark:text-brand-200">
              {initial}
            </span>
